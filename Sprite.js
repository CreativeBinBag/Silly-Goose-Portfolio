class Sprite {
    ...
    draw(ctx, cameraPerson) {
      ...
      const y = this.gameObject.y - 18 + utils.widthGrid(6) - cameraPerson.y;
         ...
         // Draw the shadow if loaded
      if (this.isShadowLoaded) {
      const shadowX = x + utils.widthGrid(0.36); // Shadow x position remains the same as character x
      const shadowY = y + utils.widthGrid(0.78); // Adjusted y coordinate for the shadow
      ctx.drawImage(this.shadow, shadowX, shadowY);
  }
